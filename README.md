...
> **Evidence status:** the interactive public demo remains a clearly labeled dated Walmart fixture replay; that replay does not invoke Solari. Separately, [credentialed run `33505918379`](https://github.com/EXO-Robotics/smartcart-solari/actions/runs/33505918379) proves the bounded owned-Demo-Grocer workflow ran Solari Browser and Solari Sandbox on implementation commit `25ab69b`. Its [sanitized receipt](evidence/live/smartcart-solari-live-proof-33505918379.json) records six timestamped Browser observations, three independently verified Sandbox decisions, enforced resource cleanup, and a `$12.79` synthetic-catalog basket. It is real Solari execution—not real-retailer pricing.
...
The key is stored only as the encrypted `SOLARI_API_KEY` Actions secret; its value is not retrievable from the repository or included in any receipt. Credentialed run `33505918379` proves the cancellation-hardened owned Demo Grocer Browser/Sandbox path and pins the exact six observed products, three selections, package counts, and `$12.79` synthetic subtotal. The bundled Walmart fixture still proves only replay contracts, validation, basket math, API/UI integration, and truthful labels. No live Walmart run occurred; retailer authorization remains a separate required gate.
