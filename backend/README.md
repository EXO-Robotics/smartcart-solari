...
| `POST` | `/mcp` | Streamable HTTP MCP endpoint exposing four intelligence tools and one native handoff action |
| `POST` | `/v1/handoffs/claim` | Exchange a short-lived encrypted bearer for a frozen reviewed-recipe payload |
| `GET` | `/t` | Universal Link target with a safe informational web fallback; the fragment bearer is not sent to the server |
