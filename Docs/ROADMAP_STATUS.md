...
| 6. Retail connectors | Shared retailer-guide engine; bounded credential-free Walmart and Target Shopping Sessions with retailer-scoped setup, one-item guidance, interruption-safe resume, outcome summaries, and pantry handoff; Kroger and other adapters truthfully gated | Human-test Walmart and Target setup, interruption/resume, retailer return, final list opening, and pantry reconciliation, then validate desired partners and request access |
| 6A. Meal Prep Mode | Beta-scoped shared `ShoppingScope`; 1–5 saved recipes, independent servings, conservative aggregation, pantry subtraction, frozen provenance, combined manifest, and the existing Retailer Assistant/pantry loop | Human-test diverse five-recipe plans, uncertain merges, relaunch recovery, Dynamic Type, and VoiceOver before broadening scope |
