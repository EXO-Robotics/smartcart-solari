...
| Raw-content retention | HTML/screenshots expose third-party or personal data | Immutable receipts keep bounded structured evidence only. The public-demo response may expose one short-lived HTTPS replay URL after cleanup; it never stores the Browser session ID and removes expired links from cached responses. | Replay retention is provider-plan dependent; keep the recorded surface owned, logged out, synthetic, and disclosure-visible. |
