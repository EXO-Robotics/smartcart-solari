...
| Cost/concurrency abuse | Spend or denial of service | Auth as appropriate, body/rate/candidate/concurrency limits, one aggregate deadline, in-flight operation cancellation, request-abort propagation, no retries on deterministic errors/429, cleanup, kill switch. | A response-socket hangup after a complete request body can run until the aggregate deadline; add response-close cancellation plus provider budgets/alerts before public live access. |
