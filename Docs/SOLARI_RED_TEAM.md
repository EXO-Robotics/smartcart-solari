...
## Public research-path red-team loop

A fresh terminal Grok pass reviewed the public self-serve route and initially scored it 8/10. It found three legitimate presentation/runtime defects: timed Browser/Sandbox progress that was not provider telemetry, a hard-coded 1.5 lb fallback in the live result narration, and research success depending on replay availability. Those were fixed. The page now shows one neutral pending state until the response identifies itself as `live` or `cached-verified-run`; the chicken tradeoff is derived from returned decisions and observations; and completed Browser/Sandbox research remains valid when recording is unavailable. Browser recording status is explicit and replay capability URLs remain fresh-caller-only.

Two initial High classifications were not accepted as implementation defects:

- Sandbox is not duplicated locally. SmartCart verifies membership, coverage, per-line arithmetic, the cheapest reference, and the premium cap, but does not recompute the cross-line global argmin or replace Sandbox's choice. The wording was tightened to make that responsibility split unmistakable.
- Signed native App Attest/TestFlight remains an honest future product gate, not a defect in the public owned-retailer demonstration. The public Browser/Sandbox route is the live provider proof; the native recordings remain explicitly `NOT LIVE`.

The post-fix fresh review pinned public head `836b8be072cb31259746f9e37731b8ac6ef18285`, runtime `0e92f408f4884343391d95ce9ee7b2acd260e188`, Vercel deployment `dpl_3yES3rNrcH1UVoHcL4wqUcNah5JY`, Pages run `33639922712`, and Cookbook head `95386ca9eee4e89781f6664b0db813795b961536`. It independently exercised the POST route and received an explicitly labeled cached verified result matching the sanitized live receipt. Its conclusion was **no legitimate Critical, High, or Medium findings**, score **9.7/10**, and **Yes** to all four required judgments: material Solari improvement within the owned-retailer use case, defensible trust, evidence-backed claims, and effectively 10/10 within the stated scope.

That score is internal development feedback, not external validation. Remaining Low items are historical V1/V3 clutter, protection on immutable Vercel deployment URLs while the public alias remains open, the DEBUG fixture's cheaper-per-line replay behavior, DNS/forwarded-IP residuals, unavailable measured provider cost, and the explicitly pending signed-device and household-use gates.
